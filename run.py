...
def render_category_scores(categories: dict[str, dict[str, Any]]) -> None:
    ...
    for category in categories.values():
        table.add_row(
            ...
            f"{int(category['weight'] * 100)}%",
            str(category["weighted_score"]),
        )
    console.print(table)
...
def render_metrics(metrics: list[dict[str, Any]]) -> None:
    ...
    for metric in metrics:
        table.add_row(metric["category"], metric["name"], metric_icon(metric["status"]), f"{metric['score']}/{metric['max_score']}", metric["details"])
    console.print(table)
...
def main(argv: list[str]) -> int:
    ...
    console.print(Panel.fit(
        ...
        f"Business type: {metadata.get('business_type', 'Unknown')}\n"
